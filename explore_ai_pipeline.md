...
**枚举事件** = 不同子功能识别出的原子判别结果。每个子系统的输出收敛为一个枚举值，与 `ExceptionDef` 规则引擎对接。

```
子功能            枚举类型              值域
─────────────────────────────────────────────────
YOLO 目标检测     EntityType            15 种（人/车/刀/枪...）
SlowFast 行为     ActionType            15 种（打架/跌倒/奔跑...）
YAMNet 音频       SoundType             15 种（枪声/尖叫/警报...）
人脸识别          FaceRecognitionResult  3 种（无脸/陌生人/已录入）
电子围栏          FenceEventType         2 种（闯入/未闯入）
```

### 当前状态

| 枚举 | 表 | 常量 | 接入 ExceptionDef |
|------|-----|------|-------------------|
| EntityType | ✅ entity_types | ✅ YOLOEntityType | ✅ M2M |
| ActionType | ✅ action_types | ✅ SlowFastActionType | ✅ M2M |
| SoundType | ✅ sound_types | ✅ YAMNetSoundType | ✅ M2M |
| FaceResult | ✅ face_recognition_results | ✅ FaceRecognitionResult | ✅ FK |
| **FenceEvent** | ❌ 缺失 | ❌ 缺失 | ❌ 缺失 |

### ExceptionDef 接入方式

电子围栏事件通过 FK 接入 ExceptionDef（与人脸识别同级）：

```
ExceptionDef
  ├─ entities (M2M → EntityType)
  ├─ actions  (M2M → SoundType)
  ├─ sounds   (M2M → ActionType)
  ├─ face_result (FK → FaceRecognitionResult)
  └─ fence_event (FK → FenceEventType)  ← 新增
```
...
电子围栏本质是"禁区判定"。YOLO 检出的人形框与预定义的围栏多边形做交集检测，配合时间窗口内的频率统计，判断是否构成"闯入"事件。

```
每帧:
  YOLO → 所有 person 框 [{track_id, bbox}]
  围栏定义 → polygon coords

  对每个 track_id:
    person_bbox ∩ fence_polygon ≠ ∅ ?
      → 写入 map[(fence_id, track_id)].append(timestamp, True|False)
```

### 滑动窗口与密度判定

维护 `(fence_id, track_id)` 为 key 的时间序列 map。保留 `{当前时间 - dwell_time} ~ {当前时间}` 窗口内的条目，超出窗口的丢弃。

```
    dwell_time = 10s, density = 0.6

帧:    T-10 T-9 T-8 T-7 T-6 T-5 T-4 T-3 T-2 T-1  NOW
重叠:    F    F   T   T   T   F   T   T   T   T   T
                                       └─ 窗口(10帧) ──┘
                                       7/10 = 0.7 ≥ 0.6 → 触发 ENTERED
```

### 状态机：防止重复触发

同一 `(fence_id, track_id)` 触发 ENTERED 后进入"已闯入"状态，持续抑制后续触发。直到 track_id 离开围栏（连续 N 帧无重叠）才重置为未闯入，允许再次触发。

```
     NOT_ENTERED ──密度达标──▶ ENTERED ──连续N帧无重叠──▶ NOT_ENTERED
```
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `dwell_time` | int (秒) | 滑动窗口大小 |
| `density` | float (0~1) | 窗口内重叠帧占比阈值 |
| `leave_frames` | int | 连续多少帧无重叠判定为"离开" |

配置粒度：围栏级别（每个围栏可独立配置）或全局默认。

### 待确认问题

1. **Person tracking 依赖**：YOLO 不跟踪，需要 ByteTrack（已在 AI 管线设计中使用）为每帧的 person 框分配 track_id。围栏逻辑完全依赖 ByteTrack 产出的 track_id。

2. **围栏绑定层级**：围栏绑定到 View（fence 加 view_id FK），还是全局？

3. **"未闯入"事件是否需要**：`NOT_ENTERED` 作为枚举值存在，但它是一个"持续状态"而非"触发事件"——每帧没有闯入的人都算未闯入。建议 FenceEventType 只保留 `ENTERED`，`NOT_ENTERED` 是默认状态，不需要作为事件写出。

### 表结构补充

```python
# models/fence_event_type.py — 新增
...
`ElectronicFence` 模型需补充字段：

```python
class ElectronicFence(Base):
    id: int (PK)
    name: str               # 新增：围栏名称
    view_id: int (FK)        # 新增：绑定到哪个 View
    coords: str (Text)       # 已有
    dwell_time: int          # 新增：停留时限（秒），默认 10
    density: float           # 新增：密度阈值，默认 0.6
    leave_frames: int        # 新增：离开判定帧数，默认 5
    created_at: datetime
```
...
```python
# In-memory per-view fence tracking map
# key: (fence_id, track_id)
# value: deque of (timestamp, overlap_bool)
fence_tracking: dict[tuple[int, int], collections.deque]
```
