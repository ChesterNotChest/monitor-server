...
- [ ] 修改 `src/service/view_task.py` 的 `get_view()` 函数
...
- [ ] 修改 `src/service/view_task.py` 的 `list_views()` 函数
  - 获取 ORM 列表后，为每个 View 调用 `build_play_urls(view.id)`
  - 返回 `list[ViewResponse]` 而非 ORM 对象列表

## 3. SRS 配置添加 CORS 支持

- [ ] 编辑 `srs/srs.conf`，在 `http_server` 块中添加 `crossdomain on;`

## 4. 验证回放流端点

- [ ] 确认 `GET /api/v1/recordings/{id}/stream` 端点存在且返回 FLV 流
- [ ] 若端点行为不正确（如只返回测试文件路径），修复之
