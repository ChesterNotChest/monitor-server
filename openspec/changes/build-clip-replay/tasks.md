...
- [ ] 1.1 更新 `src/config.py`：新增 `CACHE_DURATION_SECONDS`(30)、`RECORD_STOP_SILENCE_SECONDS`(60)

## 2. Recording 模型

- [ ] 2.1 新建 `src/models/recording.py`：Recording 模型（id, view_id FK, file_path, start_time, end_time, created_at）
- [ ] 2.2 新建 `src/repository/recording_repo.py`：按 view_id + 时间范围查询
- [ ] 2.3 更新 `src/models/__init__.py` 导入 Recording

## 3. 环形缓冲区

- [ ] 3.1 新建 `src/service/replay_module/__init__.py`
- [ ] 3.2 新建 `src/service/replay_module/ring_buffer.py`：`FrameRingBuffer` 类
...
- [ ] 4.1 新建 `src/service/replay_module/recorder.py`：`RecordingSession` 类
...
- [ ] 5.1 新建 `src/service/replay_task.py`：
...
- [ ] 6.1 新建 `src/schema/http/replay.py`：`RecordingResponse`（id, view_id, file_path, start_time, end_time）
- [ ] 6.2 新建 `src/network/api/replay.py`：
  - `GET /api/v1/views/{id}/recordings?start=&end=` — 查询录制列表
  - `GET /api/v1/recordings/{id}/stream` — 流式返回 flv 文件
- [ ] 6.3 更新 `src/network/api/__init__.py` 注册 replay_router

## 7. 测试

- [ ] 7.1 新建 `src/tests/service/test_ring_buffer.py`：测试 push/dump_all/overflow/clear
- [ ] 7.2 新建 `src/tests/service/test_recorder.py`：测试 RecordingSession 生命周期（start/on_new_alert/stop）
- [ ] 7.3 新建 `src/tests/api/test_replay_api.py`：测试 recording 查询 + stream 端点
- [ ] 7.4 运行测试：`pytest src/tests/ -v`
