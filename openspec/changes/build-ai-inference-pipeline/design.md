...
围栏是五类枚举事件中唯一使用滑动窗口+密度判定的。这不是设计缺陷——物理安防系统正是用"持续 N 秒检测到移动→报警"来过滤路过误报。窗口机制让围栏从"每帧即时触发"升级为"确认闯入后才触发"，大幅降低误报率。
...
告警引擎的 ExceptionDef 匹配 SHALL 只引用枚举值（EntityType/ActionType/SoundType/FaceRecognitionResult/FenceEventType）。各模块的原始输出（bbox、confidence、128D 向量、概率分布）SHALL 在模块内部转换为确定性枚举值后才 publish 到 EventBus。告警引擎不接触任何非枚举数据。

**理由**：复杂场景被分解为有限、确定的枚举状态组合。ExceptionDef 规则是可枚举、可测试、可调试的——每条规则由枚举值 AND 组合定义，告警回溯时能看到"当时哪些枚举活跃、哪些不活跃"。
...
- **同异常重复触发**：ALERT_COOLDOWN 期内不重复写 SituationEvent，但发 keep_alive 保持录制——避免录制因"重复异常被 suppress"而错误终止
- **围栏+Fence 排列组合不强求**：围栏作为独立禁区事件使用，不做与其他事件的复杂 AND 组合，时间尺度差异不构成实际问题
- **Fence 时间窗口是必要设计**：围栏是唯一需要滑动窗口+密度判定的枚举类型——"停留超过 N 秒才报警"是物理安防的标准做法，避免路过误报
