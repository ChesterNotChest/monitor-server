...
- **YOLO11 目标检测**（video 线性前置）：检出 person/车/刀/狗/背包等 12 类实体（COCO 预训练覆盖） → 产出 EntityType 枚举事件列表
...
- `fence-event-crud`: FenceEventType 枚举值的完整 CRUD API，与 EntityType/ActionType/SoundType 路由风格一致
...
- **新增 `src/service/vision_module/`**：vision_frame_reader/vision_annotation/vision_merger/vision_pipeline/vision_event_bus（基础设施）、vision_yolo/（YOLO）、vision_face/（人脸，Part B）、vision_slowfast/（SlowFast，Part B）、vision_fence/（电子围栏，Part B）；新增 `src/service/audio_module/`：audio_yamnet.py（YAMNet，Part C）
...
- **枚举精简**：EntityType 15→12（移除 COCO 未覆盖的 GUN/FIRE/SMOKE）；SlowFastActionType 15→12（移除无模型产出的 LOITERING/CROWDING/LYING_DOWN）
- **Pipeline 协议**：定义 `FrameContext` 数据类 + `register_frame_hook` 注册接口——A/B/C 模块通过显式接口对接，禁止隐式约定
- **枚举隔离原则**：告警引擎只消费枚举值（EntityType/ActionType/SoundType/FaceRecognitionResult/FenceEventType），不接触 bbox/128D/概率等原始数据

### CI 与测试策略

- **小数据集进 git**：COCO8（1 MB）、LFW subset（500 KB）、UrbanSound8K subset（5 MB）直接提交 `tests/fixtures/`
- **大文件走下载脚本**：PETS S1-L1（100 MB）通过 `download_weights.py` 同一模式拉取到 `third-party/`
- **CI 覆盖**：YOLO/Face/YAMNet 单元测试（模型小、秒级推理）在 CI 中跑真实推理；SlowFast CI 只测队列逻辑（mock 推理）；PETS 端到端视频验证仅本地执行
