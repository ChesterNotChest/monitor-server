...
- [x] 修改 `src/service/vision_module/vision_pipeline.py` 的 `_run_loop()`
...
- [x] 修改 `src/service/vision_task.py` 的 `start_pipeline()`
  - 在创建 AIPipeline 之后（AlertEngine 之前或之后），调用 `replay_task.start_buffer(view_id)`
  - 导入 `from src.service import replay_task`

## 3. 管线停止时清理录制缓冲区

- [x] 修改 `src/service/vision_task.py` 的 `stop_pipeline()`
...
- [x] 在 `src/service/vision_task.py` 中添加 RECORDING 事件的异步订阅者回调
...
- [x] 在 `monitor-server/.env` 中取消 `SRS_PUBLIC_HOST` 的注释并设置实际 IP
- [x] 在 `monitor-server/.env.example` 中添加注释说明不同环境应如何配置

## 6. EventResponse 添加 recording_id 字段

- [x] 修改 `src/schema/http/event.py` 的 `EventResponse`
...
- [x] 修改 `src/models/monitor_view.py` 的 `MonitorView`
  - 添加 `name: Mapped[str | None] = mapped_column(String(128), nullable=True, default=None)`
- [x] 修改 `src/schema/http/view_schema.py` 的 `ViewResponse`
  - 添加 `name: str | None = Field(None, description="视图名称")`
- [x] 修改 `src/service/view_task.py` 中三处构造 `ViewResponse` 的代码（`create_view`、`get_view`、`list_views`）
