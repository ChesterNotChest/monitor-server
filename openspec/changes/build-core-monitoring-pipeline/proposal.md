## Control/Data Plane Addendum

WSS SHALL remain the control plane for Node commands. Server uses WSS to send
bounded business commands such as stream start/stop, then verifies actual stream
availability by probing the RTMP data plane itself. Server SHALL NOT repeatedly
ask Node over WSS whether raw RTMP is ready; the source of truth is whether
Server can pull the raw RTMP URL.

Raw stream readiness waiting SHALL be bounded and configurable:

- `STREAM_READY_TIMEOUT`: total time Server may wait for all raw inputs.
- `STREAM_PROBE_TIMEOUT`: maximum time for one ffprobe attempt.
- `STREAM_READY_INTERVAL`: delay between failed probe rounds.
...
This apply step fixes the E2E failure observed after WSS command dispatch
succeeded:

- View create/delete operations must explicitly commit successful database
  lifecycle changes and roll back failed operations.
- Merge FFmpeg must not start until both raw video and audio RTMP inputs are
  reachable, because Node starts publishing asynchronously after it accepts
  `UPDATE_STREAM=true`.
- When raw inputs do not become reachable within the bounded wait, Server
  returns the created View with warnings and does not spawn merge FFmpeg.
