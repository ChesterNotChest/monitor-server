...
**Purpose:** 定义音频采集设备数据模型，记录设备信息、所属计算节点及推流状态。设备名称在单个 Node 下唯一，不同 Node 可有同名设备。
