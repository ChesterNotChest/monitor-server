...
_fence_labels: dict[int, dict[int, str]] = {}  # view_id → {track_id → label}
...
_active_action_type_ids: dict[int, frozenset[int]] = {}  # view_id → action_ids
_active_action_ids_updated_at: dict[int, float] = {}     # view_id → timestamp
_active_sound_type_ids: dict[int, frozenset[int]] = {}   # view_id → sound_ids
_active_sound_ids_updated_at: dict[int, float] = {}      # view_id → timestamp
...
def get_active_signals(
    ...
    *,
    ...
    # 动作 TTL 检查（SlowFast 推理是批处理，结果跨帧保持，按 View 隔离）
    action_ids: frozenset[int]
    action_updated = _active_action_ids_updated_at.get(view_id, 0)
    if action_updated > 0 and _time.time() - action_updated < _SIGNAL_TTL:
        action_ids = _active_action_type_ids.get(view_id, frozenset())
    else:
        action_ids = frozenset()

    # 声音 TTL 检查（按 View 隔离）
    sound_ids: frozenset[int]
    sound_updated = _active_sound_ids_updated_at.get(view_id, 0)
    if sound_updated > 0 and _time.time() - sound_updated < _SIGNAL_TTL:
        sound_ids = _active_sound_type_ids.get(view_id, frozenset())
    else:
        sound_ids = frozenset()
    ...
    # Fence: 解析 label 后缀提取结果类型（按 View 隔离）
    from src.constants import FenceEventResult
    fence_ids_set: set[int] = set()
    for label in _fence_labels.get(view_id, {}).values():
...
async def _on_fence_event(payload: dict) -> None:
    ...
    view_id: int = payload.get("view_id", 0)
    logger.info("[FenceSub] called, view=%d fences=%s", view_id, len(fences))
    if view_id not in _fence_labels:
        _fence_labels[view_id] = {}
    view_labels = _fence_labels[view_id]
    for f in fences:
        tid = f.get("track_id")
        if tid is None:
            continue
        ...
        if entered:
            ...
            view_labels[tid] = f"Fence-{fid}{suffix}"
        else:
            view_labels.pop(tid, None)
