...
class TestExceptionDef:
    ...
    def test_create_and_get(self, db):
        ...
        exc = create_exception(db, SeverityLevel.CRITICAL, g.id)
    ...
    def test_list_all(self, db):
        g = self._setup_group(db)
        create_exception(db, SeverityLevel.WARNING, g.id)
        items, total = list_exceptions(db)
        assert total >= 1

    def test_list_by_severity(self, db):
        g = self._setup_group(db)
        create_exception(db, SeverityLevel.EMERGENCY, g.id)
    ...
    def test_update(self, db):
        ...
        exc = create_exception(db, SeverityLevel.INFO, g1.id)
    ...
    def test_delete(self, db):
        g = self._setup_group(db)
        exc = create_exception(db, SeverityLevel.WARNING, g.id)
